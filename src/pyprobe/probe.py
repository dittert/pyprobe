...
def _init_state_path(cmd_options):
    ...
    if platform.system() == "Darwin":
        ...
        default_location = "/var/run/pyprobe/pyprtg.state"
    else:
...
def _register_probe():
    ...
    for s in _sensors.values():
        ...
        try:
            ...
            print "Error calling define() for sensor kind {}. Sensor was ignored.".format(s.KIND)
            traceback.print_exc()
    ...
    for i in xrange(3):
        try:
            ...
            print("Could not register probe with server.")
...
def _query_tasks():
    ...
    for i in xrange(3):
        try:
            ...
            print("Failed to retrieve tasks from server.")
            traceback.print_exc()
            return []
        except Timeout:
            pass
...
def _publish_results(resultlist):
    ...
    for i in xrange(3):
        try:
            ...
            _check_access_to_server(res)
            print("Published {} results to PRTG.".format(len(resultlist)))
            break
        except ConnectionError as ex:
            raise ProbeException(ex.message)
        except Timeout:
        ...
        print("{} results were lost because the server could not be reached.".format(len(resultlist)))
...
def main():
    try:
        ...
        try:
            ...
            print("Probe was not authorized or banned on core server. Unable to continue")
            sys.exit(1)
